...
coqdep's symlink-following directory walk. Calling `rocq dep` as a subprocess
instead would be worse than either: on a `Require` that matches several files
it picks one, prefers the last binding on the command line, which is the
solver's, prints a warning on stderr and exits 0, so the ambiguity the plan
rejects would have to be recovered from prose. The copy is licensed under the
LGPL and listed in `NOTICE`; refresh it when the switch's Rocq changes.
