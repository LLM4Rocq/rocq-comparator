...
(* rocq-comparator: this file is tools/coqdep/lib/lexer.mll of Rocq 9.2.0,
   unchanged, under its own LGPL 2.1 licence.  It is the lexer "rocq dep"
   uses to find the Require statements of a .v file.  It is vendored rather
   than linked from rocq-runtime.coqdeplib because that library registers a
   warning name at initialisation that the Rocq library itself owns, so the
   two cannot be linked into one process. *)
